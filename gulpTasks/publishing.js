...
gulp.task('upload-binaries', cb => {
  ...
  got(
    `https://api.github.com/repos/ethercore/desktop-wallet/releases?access_token=${GITHUB_TOKEN}`,
    { json: true }
  )
    ...
    .then(draft => {
      ...
      if (draft.draft === true) {
        ...
        return (
          githubUpload({
            url: `https://uploads.github.com/repos/ethercore/desktop-wallet/releases/${
              draft.id
            }/assets{?name}`,
            token: [GITHUB_TOKEN],
            assets: filePaths
            ...
            .then(() => {
              ...
              if (draft.body && checksums) {
                const checksumRows = checksums
                  ...
                  .join('\n');
                got.patch(
                  `https://api.github.com/repos/ethercore/desktop-wallet/releases/${
                    draft.id
